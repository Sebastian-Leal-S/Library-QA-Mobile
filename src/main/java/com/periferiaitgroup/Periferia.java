...
public class Periferia {
    ...
    public static void scrollVertical( int xInit, int yInit, int yFinal, int iteraciones) {
        for (int i = 1 ; i<=iteraciones; i++) {
        	...
        	Sequence swipe = new Sequence(finger, 1)
                    ...
                    .addAction(finger.createPointerDown(PointerInput.MouseButton.LEFT.asArg()))
                    .addAction(new Pause(finger, Duration.ofSeconds(3)))
                    .addAction(finger.createPointerMove(Duration.ofMillis(500), PointerInput.Origin.viewport(), xInit, yFinal))
                    .addAction(finger.createPointerUp(PointerInput.MouseButton.LEFT.asArg()));
	 ...
	 * @throws IOException
	 */
	public static void scroll(int yInicial, int xInicial, int yFinal, int cantRepet)
			throws IOException {
